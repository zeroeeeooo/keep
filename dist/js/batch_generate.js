...
async function startBatchGenerate() {
    ...
    let weatherMode = document.getElementById("batch_weather_mode").value; // "fixed" | "random" | "api"
    ...
    // API 天气模式：提前读取 API Key
    let apiKey = null;
    if (weatherMode === "api") {
        let apiKeyInput = document.getElementById("inpt_amap_key");
        apiKey = apiKeyInput ? apiKeyInput.value.trim() : "";
        if (!apiKey) {
            alert("请先在「高德地图API Key」输入框中填写 API Key，再使用 API 天气模式。");
            return;
        }
    }

    // 上次成功获取的 API 天气（失败降级用）
    let lastApiWeather = null;
    ...
    try {
        for (let i = 0; i < count; i++) {
            ...
            // ---- 设置天气（API 模式：每张图独立调用）----
            if (weatherMode === "api" && apiKey) {
                showBatchMessage("正在生成: " + (i + 1) + " / " + count + " (获取实时天气...)");
                try {
                    // 两张图之间加 800ms 间隔，控制调用频率
                    if (i > 0) await delay(800);
                    let apiWeather = await loadAMapWeather(apiKey);
                    lastApiWeather = apiWeather;
                    setWeather(apiWeather.weather);
                    temperature = parseInt(apiWeather.temperature) || 0;
                    humidity = parseInt(apiWeather.humidity) || 50;
                    document.getElementById("inpt_temperature").value = temperature;
                    document.getElementById("inpt_humidity").value = humidity;
                } catch (err) {
                    console.warn("第 " + (i + 1) + " 张 API 天气获取失败:", err);
                    if (lastApiWeather) {
                        // 降级：用上次成功数据 + 波动
                        setWeather(lastApiWeather.weather);
                        temperature = parseInt(lastApiWeather.temperature) + Math.floor(Math.random() * 5) - 2;
                        humidity = parseInt(lastApiWeather.humidity) + Math.floor(Math.random() * 11) - 5;
...
function batchWeatherModeChange() {
    ...
    let apiTip = document.getElementById("batch_weather_api_tip");

    if (mode === "fixed") {
        fixedWrap.style.display = "inline";
        if (apiTip) apiTip.style.display = "none";
    } else if (mode === "api") {
        fixedWrap.style.display = "none";
        if (apiTip) apiTip.style.display = "inline";
    } else {
        fixedWrap.style.display = "none";
        if (apiTip) apiTip.style.display = "none";
    }
